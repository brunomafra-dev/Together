...
import { ArrowRight, Check, Heart, Lock, Mail, MoveRight, ShieldQuestion } from "lucide-react";
...
const highlights = [
  ...
  "Projeção financeira do mês",
];

export default function App() {
  const [email, setEmail] = useState("voces@email.com");
  const [password, setPassword] = useState("12345678");

  const maskedPassword = useMemo(() => "•".repeat(Math.max(8, password.length)), [password]);
            ...
            <section className="mt-10 overflow-hidden rounded-[2rem] border border-[#e6e2dd] bg-white shadow-[0_22px_60px_rgba(15,23,42,0.07)]">
              <div className="px-6 pb-6 pt-9 sm:px-8">
                <form className="space-y-6 text-left">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <Input
                        ...
                        readOnly
                        className="h-[54px] rounded-2xl border-[#e5e7eb] bg-white pl-11 pr-4 text-[1rem] tracking-[0.24em] text-[#334155] shadow-none placeholder:text-[#94a3b8] focus-visible:ring-0"
                      />
                    </div>
                  </div>
                  ...
                  <Button
                    type="button"
                    className="h-[48px] w-full rounded-2xl bg-[#00a56f] text-[1.02rem] font-semibold text-white shadow-[0_14px_28px_rgba(0,165,111,0.24)] hover:bg-[#0b9a69]"
                  >
                    Entrar
              ...
              <div className="border-t border-[#e6e2dd] px-6 py-7 text-center sm:px-8">
                <p className="text-[0.98rem] text-[#6b7280]">
                  Primeira vez aqui?{" "}
                  <a href="#" className="font-semibold text-[#0f766e] hover:underline">
                    Criar conta
                  </a>
                </p>
              </div>
