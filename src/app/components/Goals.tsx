...
function emptyGoal(income: number): GoalSnapshot {
  return {
    ...
    planCards: [
      { name: "Moradia", share: "0%", amount: 0, tone: "stone" },
      { name: "Alimentação", share: "0%", amount: 0, tone: "emerald" },
      { name: "Investimentos", share: "0%", amount: 0, tone: "cyan" },
      { name: "Lazer", share: "0%", amount: 0, tone: "amber" },
      { name: "Outros", share: "0%", amount: 0, tone: "indigo" },
    ],
    progressRows: [
    ...
    suggestion: {
      ...
      note:
        "A divisão abaixo reflete a base atual de vocês. Ela pode servir como ponto de partida, mas o casal/família continua livre para ajustar como quiser.",
      allocations: [
...
function buildSuggestedPlan(income: number): PlanCard[] {
  return [
    { name: "Moradia", share: "35%", amount: income * 0.35, tone: "stone" },
    { name: "Alimentação", share: "20%", amount: income * 0.2, tone: "emerald" },
    { name: "Investimentos", share: "15%", amount: income * 0.15, tone: "cyan" },
    { name: "Lazer", share: "10%", amount: income * 0.1, tone: "amber" },
    { name: "Outros", share: "20%", amount: income * 0.2, tone: "indigo" },
  ];
}

function SectionHeader({ icon: Icon, title, actionLabel }: { icon: typeof Target; title: string; actionLabel?: string }) {
  return (
...
export function Goals() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const progressRows = await financeService.fetchGoalProgressRows(currentGoal.id).catch(() => []);

      const income = settings.monthlyIncome || household.monthlyIncome || 0;
      const averagePlanUsage = progressRows.length
        ? Math.round(
            progressRows.reduce((sum, row) => sum + (row.planned > 0 ? (row.realized / row.planned) * 100 : 0), 0) /
              progressRows.length,
          )
        : 0;
      ...
      setGoal({
        ...
        householdIncome: income,
        planCards: planItems.length
          ? planItems.map((item) => ({ name: item.name, share: item.share, amount: item.amount, tone: item.tone }))
          : buildSuggestedPlan(income),
        progressRows: progressRows.length
          ? progressRows.map((row) => ({ name: row.name, planned: row.planned, realized: row.realized, status: row.status }))
          : emptyGoal(income).progressRows,
        financialHealth: {
          score: averagePlanUsage,
    ...
    void load();
  }, [household?.id, household?.monthlyIncome, settings.monthlyIncome]);
      ...
      <div className="space-y-7">
        <header className="space-y-2">
          ...
          <p className="text-xs text-stone-500">
            Dados reais do Supabase quando disponíveis. Caso contrário, a tela fica zerada e mostra apenas uma sugestão de base saudável.
          </p>
        </header>

        <section className="space-y-4">
          <SectionHeader icon={Target} title="Objetivo principal" actionLabel="Editar objetivo" />
        ...
        <section className="space-y-4">
          <SectionHeader icon={Wallet} title="Planejamento do casal" actionLabel="Ajustar plano" />
