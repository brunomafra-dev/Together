...
type ContributionTarget = { kind: "main" } | { kind: "future"; index: number };
...
function emptyGoal(income: number): GoalSnapshot {
  return {
    ...
    insights: [
      ...
      {
        kind: "positive",
        text: "A divisão inicial é uma sugestão editável para ajudar o casal a começar.",
      },
    ],
    suggestion: {
...
export function Goals() {
  ...
  const mainGoalPercent =
    snapshot.total > 0 ? Math.round((snapshot.current / snapshot.total) * 100) : 0;
  ...
  const financialData = useMemo(() => {
    ...
    const healthScore =
      realIncome > 0
        ? Math.max(0, Math.min(100, Math.round((remainingBalance / realIncome) * 100)))
        : 0;
    const spendingTrend =
  ...
  const planningTotalPercent = planningAllocations.reduce(
      ...
      setGoal({
        ...
        progressRows: rows.length
          ? rows.map((row) => ({
              ...
              status:
                row.status === "Sem meta cadastrada" || !row.status ? "Em andamento" : row.status,
            }))
          : [],
        financialHealth: {
          score: rows.length
            ? Math.round(
                rows.reduce(
                  (sum, row) => sum + (row.planned > 0 ? (row.realized / row.planned) * 100 : 0),
                  0,
                ) / rows.length,
              )
            : 0,
        ...
        monthRadar: {
          ...
          note:
            rows.length > 0
              ? `${rows.length} meta${rows.length === 1 ? "" : "s"} futura${rows.length === 1 ? "" : "s"} acompanhando esse objetivo.`
              : "Adicione metas futuras para dividir esse objetivo em etapas menores.",
        },
        insights: [],
  ...
  const refreshGoalState = async (
    ...
    setGoal({
      ...
      monthRadar: {
        label: "Radar do mês",
        summary:
          savedGoal.targetAmount > savedGoal.currentAmount
  ...
  const handleDeleteProgressRow = async (rowIndex: number) => {
    ...
    try {
      ...
      setFormError(
        error instanceof Error ? error.message : "Não foi possível excluir a meta futura.",
      );
    } finally {
      setSaving(false);
    }
  ...
  const handleSaveFutureGoal = async () => {
    ...
    try {
      ...
      const currentRow =
        editingFutureGoalIndex !== null && editingFutureGoalIndex >= 0
          ? progressRows[editingFutureGoalIndex]
          : null;
      if (currentRow?.id) {
      ...
      setFormError(
        error instanceof Error ? error.message : "Não foi possível salvar a meta futura.",
      );
    } finally {
      setSaving(false);
    }
    ...
    <Layout>
      <div className="space-y-5 sm:space-y-7">
        <header className="space-y-1.5 sm:space-y-2">
          ...
          <p className="hidden text-sm leading-6 text-stone-600 sm:block">
            Como o ritmo de vocês conversa com os planos do casal
          </p>
        </header>

        <section className="grid gap-3 sm:gap-4 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="rounded-2xl border border-emerald-100 bg-gradient-to-br from-emerald-50 to-white p-3.5 shadow-sm sm:rounded-[2rem] sm:p-6">
            <div className="flex items-start justify-between gap-3 sm:gap-4">
              <div className="min-w-0 space-y-1.5 sm:space-y-3">
                <div className="inline-flex items-center gap-1.5 rounded-full bg-emerald-100 px-2 py-0.5 text-[11px] font-medium text-emerald-700 sm:gap-2 sm:px-3 sm:py-1 sm:text-sm">
                  <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 sm:h-2.5 sm:w-2.5" />
                  Saúde financeira
                </div>
                <h3 className="break-words text-base font-semibold text-stone-950 sm:text-2xl sm:font-medium">
                  {formatBRL(financialData.remainingBalance)} livres neste mês
                </h3>
                <p className="hidden text-sm leading-6 text-stone-600 sm:block">
                  Renda real {formatBRL(financialData.realIncome)}
                  {financialData.extraIncome > 0
                    ? ` (${formatBRL(income)} planejada + ${formatBRL(financialData.extraIncome)} entradas)`
                    : ""}
                  {" - "}fixas {formatBRL(financialData.fixedSpent)} - variáveis{" "}
                  {formatBRL(financialData.variableSpent)} - parcelas{" "}
                  {formatBRL(financialData.installmentSpent)}.
                </p>
              </div>
              <div className="shrink-0 text-right">
                <div className="break-words text-2xl font-semibold text-teal-700 sm:text-5xl">
                  {financialData.healthScore}%
                </div>
                <p className="text-[11px] text-stone-500 sm:text-sm">da renda livre</p>
              </div>
            </div>
          </div>

          <div className="rounded-2xl border border-emerald-100 bg-emerald-50/70 p-3.5 shadow-sm sm:rounded-[2rem] sm:p-6">
            <div className="flex items-center gap-2.5 sm:gap-3">
              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-emerald-100 text-emerald-700 sm:h-12 sm:w-12 sm:rounded-2xl">
                <Sparkles className="h-3.5 w-3.5 sm:h-5 sm:w-5" />
              </div>
              <div className="min-w-0">
                <h3 className="text-base font-semibold text-stone-950 sm:text-xl sm:font-medium">
                  Radar do mês
                </h3>
                <p className="hidden text-sm leading-6 text-stone-600 sm:block">
                  Categorias, tendência e saldo projetado
                </p>
              </div>
            </div>
            <div className="mt-3 grid grid-cols-3 gap-2 sm:mt-5 sm:gap-3">
              <div className="min-w-0 rounded-xl border border-emerald-100 bg-white/70 p-2 sm:rounded-2xl sm:p-4">
                <p className="text-[11px] uppercase tracking-[0.14em] text-stone-500 sm:text-xs sm:tracking-[0.16em]">
                  Maior gasto
                </p>
                <p className="mt-1 truncate text-xs font-semibold text-stone-950 sm:mt-2 sm:break-words sm:text-sm">
                  {financialData.highestCategory?.name ?? "Sem gastos"}
                </p>
                <p className="break-words text-[11px] text-stone-600 sm:text-sm">
                  {formatBRL(financialData.highestCategory?.amount ?? 0)}
                </p>
              </div>
              <div className="min-w-0 rounded-xl border border-emerald-100 bg-white/70 p-2 sm:rounded-2xl sm:p-4">
                <p className="text-[11px] uppercase tracking-[0.14em] text-stone-500 sm:text-xs sm:tracking-[0.16em]">
                ...
                <p className="mt-1 truncate text-xs font-semibold text-stone-950 sm:mt-2 sm:break-words sm:text-sm">
                  {financialData.lowestCategory?.name ?? "Sem comparação"}
                </p>
                <p className="break-words text-[11px] text-stone-600 sm:text-sm">
                  {formatBRL(financialData.lowestCategory?.amount ?? 0)}
                </p>
              </div>
              <div className="min-w-0 rounded-xl border border-emerald-100 bg-white/70 p-2 sm:rounded-2xl sm:p-4">
                <p className="text-[11px] uppercase tracking-[0.14em] text-stone-500 sm:text-xs sm:tracking-[0.16em]">
                ...
                <p className="mt-1 truncate text-xs font-semibold text-stone-950 sm:mt-2 sm:break-words sm:text-sm">
                  {formatBRL(financialData.projectedEndBalance)}
                </p>
                <p className="hidden break-words text-sm text-stone-600 sm:block">
                  {financialData.spendingTrend}
                </p>
              </div>
            </div>
          </div>
        ...
        <section className="space-y-2.5 sm:space-y-4">
          <SectionHeader
            ...
            actionLabel={goal ? "Editar objetivo" : "Criar objetivo"}
            onAction={openGoalEdit}
          />
          <div className="rounded-2xl border border-stone-200 bg-gradient-to-br from-white via-emerald-50/40 to-emerald-50 p-3.5 shadow-sm sm:rounded-[2rem] sm:p-6">
            <div className="flex items-start justify-between gap-3 lg:gap-4">
              <div className="min-w-0 space-y-2 sm:space-y-4">
                <div className="inline-flex max-w-full break-words rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-medium uppercase tracking-[0.12em] text-emerald-700 sm:px-3 sm:py-1 sm:text-xs sm:tracking-[0.18em]">
                  {snapshot.label}
                </div>
                <div>
                  <h2 className="break-words text-lg font-semibold text-stone-950 sm:text-3xl">
                    {snapshot.title}
                  </h2>
                  <p className="mt-1 break-words text-xs text-stone-600 sm:mt-2 sm:text-sm">
                    {goal
                      ? `${formatBRL(snapshot.current)} de ${formatBRL(snapshot.total)}`
                      : "Sem meta cadastrada"}
                  </p>
                </div>
                {goal && (
                  <button
                    ...
                    className="inline-flex w-fit items-center gap-1.5 rounded-lg border border-emerald-200 bg-white px-2.5 py-1.5 text-xs font-medium text-emerald-700 transition-colors hover:bg-emerald-50 sm:gap-2 sm:rounded-xl sm:px-3 sm:py-2 sm:text-sm"
                  >
                ...
                <div className="w-full max-w-3xl">
                  <div className="h-2.5 rounded-full bg-stone-100 sm:h-3">
                    <div
                      className="h-2.5 rounded-full bg-gradient-to-r from-emerald-500 to-teal-500 sm:h-3"
                      style={{ width: `${mainGoalPercent}%` }}
                    />
                  </div>
                  <p className="hidden mt-3 break-words text-sm text-stone-600 sm:block">
                    {goal
                      ? `Faltam ${formatBRL(mainGoalRemaining)} para completar.`
                      : "Cadastre uma meta para ver progresso e projeções."}
                  </p>
                </div>
              </div>
              <div className="shrink-0 text-right lg:min-w-[140px]">
                <div className="break-words text-2xl font-semibold text-teal-700 sm:text-5xl">
                  {mainGoalPercent}%
                </div>
                <p className="mt-0.5 text-[11px] text-stone-500 sm:mt-1 sm:text-sm">concluído</p>
              </div>
            </div>
          </div>
        </section>
        ...
        >
          <p className="hidden text-sm leading-6 text-stone-600 sm:block">
            Como vocês decidiram dividir a renda mensal de {formatBRL(snapshot.householdIncome)}
          </p>
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {planCards.map((card) => (
              <article
                key={card.id ?? card.localId ?? card.name}
                className={`rounded-2xl border p-4 ${toneClass(card.tone)}`}
              >
                {(() => {
                  const spent =
                  ...
                  return (
                    <>
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <span className="text-[11px] text-stone-500 sm:text-sm">{card.share}</span>
                      </div>
                      <p className="mt-5 break-words text-xl font-semibold sm:text-2xl">
                      ...
                      <div className="mt-6 h-2 rounded-full bg-white/70">
                        <div
                          ...
                          style={{
                            width: `${card.amount > 0 ? Math.min((spent / card.amount) * 100, 100) : 0}%`,
                          }}
                        />
                      </div>
                    </>
          ...
          <div className="overflow-hidden rounded-[2rem] border border-stone-200 bg-white shadow-sm">
            {progressRows.length > 0 ? (
              progressRows.map((row, index) => {
                ...
                return (
                  <div
                    key={row.id ?? row.localId ?? `future-goal-${index}`}
                    className={`p-5 ${index !== progressRows.length - 1 ? "border-b border-stone-100" : ""}`}
                  >
                    <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                      <div className="min-w-0 space-y-2">
                        <div className="flex items-start gap-2">
                          <h3 className="min-w-0 break-words text-lg font-medium text-stone-950">
                            {row.name}
                          </h3>
                        ...
                        <p className="break-words text-sm text-stone-500">
                          Alvo: {formatBRL(row.planned)} · Atual: {formatBRL(row.realized)}
                        </p>
                      </div>
                    ...
                    <p className="mt-3 text-sm text-stone-600">
                      {diff > 0
                        ? `Faltam ${formatBRL(diff)} para esta meta futura.`
                        : diff < 0
                          ? `${formatBRL(Math.abs(diff))} acima do alvo.`
                          : "Meta futura no valor alvo."}
                    </p>
                  </div>
                );
              })
      ...
      {editingGoal && (
        ...
        >
          <div
            className="w-full max-w-3xl max-h-[calc(100vh-2rem)] overflow-hidden rounded-3xl bg-white shadow-2xl"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <div className="max-h-[calc(100vh-8rem)] overflow-y-auto px-6 py-5">
              ...
              <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-end">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-xl bg-stone-900 px-4 py-2.5 text-sm font-medium text-white disabled:opacity-60"
                >
      ...
      {editingFutureGoalIndex !== null && (
        <div
          className="fixed inset-0 z-50 flex items-end justify-center overflow-y-auto bg-black/50 p-4 sm:items-center"
          onClick={closeFutureGoalEdit}
        >
          <div
            className="max-h-[calc(100vh-2rem)] w-full max-w-md overflow-y-auto rounded-2xl bg-white p-5 shadow-lg sm:p-6"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="mb-4 flex items-center justify-between gap-3">
            ...
            <div className="space-y-2.5 sm:space-y-4">
              ...
              <div className="flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-end">
                ...
                <button
                  type="button"
                  disabled={saving}
                  onClick={() => void handleSaveFutureGoal()}
                  className="inline-flex items-center justify-center gap-2 rounded-xl bg-stone-900 px-4 py-2.5 text-sm font-medium text-white disabled:opacity-60"
                >
                  <Save className="h-4 w-4" />
                  {saving ? "Salvando..." : "Salvar meta"}
      ...
      {editingPlanning && (
        ...
        >
          <div
            className="max-h-[100dvh] w-full max-w-3xl overflow-hidden rounded-t-3xl bg-white shadow-2xl sm:max-h-[calc(100vh-2rem)] sm:rounded-3xl"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="sticky top-0 z-10 flex items-center justify-between gap-3 border-b border-stone-100 bg-white px-5 py-4 sm:px-6">
              <div className="min-w-0">
                ...
                <p className="mt-1 text-xs text-stone-500 sm:hidden">
                  Edite uma categoria por vez. Novas categorias aparecem no final.
                </p>
              </div>
              <button
                type="button"
                    ...
                    >
                      <div className="mb-3 flex items-start justify-between gap-3">
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            setPlanningAllocations((prev) => prev.filter((_, i) => i !== index))
                          }
