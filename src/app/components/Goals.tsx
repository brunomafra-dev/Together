...
export function Goals() {
  ...
  const syncDraftFromSnapshot = (source: GoalSnapshot) => {
    setTitle(source.title);
    setLabel(source.label);
    setCurrent(String(source.current));
    setTotal(String(source.total || income || 0));
    setAllocations(source.planCards.length ? source.planCards : buildSuggestedPlan(income));
    setProgressRows(source.progressRows);
  };
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (!currentGoal) {
        ...
        setGoal(null);
        syncDraftFromSnapshot(fallback);
        return;
      }
      ...
      const hydratedGoal: GoalSnapshot = {
        id: currentGoal.id,
        ...
        insights: rows.length
          ? rows.map((row) => ({
              kind: "info",
              text: `${row.name} está ${row.planned - row.realized >= 0 ? "abaixo" : "acima"} do planejado.`,
            }))
      ...
      syncDraftFromSnapshot(hydratedGoal);
    };
  ...
  const openEdit = () => {
    syncDraftFromSnapshot(goal ?? emptyGoal(income));
    setFormError(null);
    setEditing(true);
  };
  ...
  const handleSave = async () => {
    ...
    if (!householdId) {
      setFormError("Aguarde o carregamento do household antes de salvar.");
      return;
    }
    ...
    try {
      ...
      const savedGoal = goal?.id ? await financeService.updateGoal(goal.id, payload) : await financeService.addGoal(payload);
      ...
      <div className="space-y-7">
        <header className="space-y-2">
          ...
          <p className="text-xs text-stone-500">
            Dados reais do Supabase quando disponíveis. Caso contrário, a tela fica zerada e mostra apenas uma sugestão de base
            saudável.
          </p>
        </header>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="mt-2 text-sm text-stone-600">
                    {snapshot.total > 0 ? `${formatBRL(snapshot.current)} de ${formatBRL(snapshot.total)}` : "Sem meta cadastrada"}
                  </p>
                </div>
                <div className="w-full max-w-3xl">
                  ...
                  <p className="mt-3 text-sm text-stone-600">
                    {snapshot.total > 0 ? `Faltam ${formatBRL(mainGoalRemaining)} para completar.` : "Cadastre uma meta para ver progresso e projeções."}
                  </p>
                </div>
              </div>
            ...
            {planCards.map((card, index) => (
              <article key={card.id ?? card.name} className={`rounded-2xl border p-4 ${toneClass(card.tone)}`}>
                <div className="flex items-start justify-between gap-3">
                  <h3 className="min-w-0 text-base font-medium">{card.name}</h3>
                  <span className="shrink-0 text-sm text-stone-500">{card.share}</span>
                </div>
                  ...
                  {editing && (
                    <div className="mt-4 grid gap-3 md:grid-cols-3">
                      <input
                        ...
                        className="rounded-xl border border-stone-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500"
                        placeholder="Nome"
                      />
                      <input
                        type="number"
            ...
            {snapshot.insights.map((item) => (
              <div
                ...
                className={`rounded-2xl border px-4 py-4 shadow-sm ${
                  item.kind === "positive" ? "border-emerald-100 bg-emerald-50 text-emerald-950" : "border-sky-100 bg-sky-50 text-sky-900"
                }`}
              >
          ...
          >
            <div className="shrink-0 border-b border-stone-100 px-6 py-4">
              <div className="flex items-center justify-between">
                ...
                <button type="button" onClick={closeEdit} className="text-stone-400 hover:text-stone-700">
                  <X className="h-5 w-5" />
                </button>
              </div>
            </div>
            ...
            >
              ...
              <div className="mt-6 grid gap-4 lg:grid-cols-2">
                <div className="rounded-2xl border border-stone-200 p-4">
                  ...
                  <div className="space-y-3">
                    {allocations.map((item, index) => (
                      <div key={item.id ?? item.name} className="grid grid-cols-[minmax(0,1.5fr)_0.7fr_minmax(0,1fr)] gap-2">
                        <input
                          value={item.name}
                          onChange={(e) => updateAllocation(index, { name: e.target.value })}
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateAllocation(index, {
                              percent: Number(e.target.value) || 0,
                              share: buildPercentText(Number(e.target.value) || 0),
                              amount: (Number(total) || income) * ((Number(e.target.value) || 0) / 100),
                            })
                          }
                          className="min-w-0 rounded-xl border border-stone-200 px-3 py-2 text-sm"
                        />
                        <div className={`rounded-xl border px-3 py-2 text-sm ${toneClass(item.tone)}`}>
                          {formatBRL((Number(total) || income) * (item.percent / 100))}
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="rounded-2xl border border-stone-200 p-4">
                  ...
                  <div className="space-y-3">
                    {progressRows.map((row, index) => (
                      <div key={row.id ?? row.name} className="grid grid-cols-[minmax(0,1.2fr)_0.8fr_0.8fr] gap-2">
                        <input
                          value={row.name}
                          onChange={(e) => updateProgressRow(index, { name: e.target.value })}
                          className="min-w-0 rounded-xl border border-stone-200 px-3 py-2 text-sm"
              ...
              <div className="mt-6 flex items-center justify-end gap-3">
                <button type="button" onClick={closeEdit} className="rounded-xl border border-stone-200 px-4 py-2.5 text-sm text-stone-700">
                  Cancelar
                </button>
                <button
                  type="submit"
