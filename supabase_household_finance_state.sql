...
drop policy if exists "household_finance_state_insert_member" on public.household_finance_state;
drop policy if exists "household_finance_state_update_member" on public.household_finance_state;
drop policy if exists "household_finance_state_delete_member" on public.household_finance_state;
...
-- State transitions are RPC-only. Keeping this legacy script read-only avoids
-- bypassing the cycle close/reopen transaction installed by the current
-- manual-cycle migration.
